...
with tab1:
    ...
    if not inbound_df.empty or not outbound_df.empty:
        ...
        daily_progress_result = create_daily_progress_table(inbound_df)
        
        if not daily_progress_result:
            st.info("Belum ada data kedatangan")
...
with tab2:
    ...
    def render_order_card(supplier, animal_type, variant, ordered_qty, delivered_qty, outbound_qty, remaining_qty, delivery_count, completion_rate, inbound_df, selected_general_animal):
        ...
        with st.container():
            ...
            with st.container():
                ...
                with col1:
                    st.markdown(f"**Dipesan**<br>{int(ordered_qty)}", unsafe_allow_html=True)
                    st.markdown(f"**Diterima**<br>{int(delivered_qty)}", unsafe_allow_html=True)
                
                with col2:
                    st.markdown(f"**Sisa**<br>{int(remaining_qty)}", unsafe_allow_html=True)
                    st.markdown(f"**Keluar**<br>{int(outbound_qty)}", unsafe_allow_html=True)
                
                with col3:
                    st.markdown(f"**Kirim**<br>{int(delivery_count)}", unsafe_allow_html=True)
                # Kedatangan information as text
                st.markdown("**Kedatangan**")
                
                # Get daily arrivals for this specific supplier and variant
                daily_arrivals = get_daily_arrivals(
                    inbound_df,
                    supplier=supplier,
                    animal_type=animal_type,
                    variant=variant,
                    selected_animal=selected_general_animal
                )
                
                if not daily_arrivals.empty:
                    # Display as text in columns instead of table
                    kedatangan_cols = st.columns(min(len(daily_arrivals), 5))  # Max 5 columns
                    
                    for idx, (_, row) in enumerate(daily_arrivals.head(5).iterrows()):
                        date_str = row['date'].strftime('%d/%m') if hasattr(row['date'], 'strftime') else str(row['date'])
                        with kedatangan_cols[idx]:
                            st.markdown(f"**{date_str}** | {int(row['quantity'])} ekor", unsafe_allow_html=True)
                else:
                    st.markdown("*Belum ada kedatangan*")
    ...
    def render_vendor_summary_card(supplier, animal_type, filtered_data, inbound_df, selected_general_animal):
        ...
        with st.container():
            ...
            with col1:
                st.markdown(f"**Dipesan**<br>{int(total_ordered)}", unsafe_allow_html=True)
            
            with col2:
                st.markdown(f"**Diterima**<br>{int(total_delivered)}", unsafe_allow_html=True)
            
            with col3:
                st.markdown(f"**Sisa**<br>{int(total_remaining)}", unsafe_allow_html=True)
            
            with col4:
                st.markdown(f"**Keluar**<br>{int(total_outbound)}", unsafe_allow_html=True)
            
            with col5:
                st.markdown(f"**Kirim**<br>{int(total_delivery_count)}", unsafe_allow_html=True)
            
            # Kedatangan information as text
            st.markdown("**Kedatangan**")
            
            # Get daily arrivals for this vendor
            daily_arrivals = get_daily_arrivals(
                inbound_df,
                supplier=supplier,
                animal_type=animal_type if animal_type != "Semua" else None,
                selected_animal=selected_general_animal
            )
    ...
    if not inbound_df.empty or not outbound_df.empty:
        ...
        if not order_data.empty:
            ...
            with col3:
                status_options = ['Semua', '🟢 Lengkap', '🟡 Sebagian', '🔴 Pending']
